...
| Earlier Ubuntu source snapshot, including native terminal input and internal relay-credential isolation |302 Python tests ran:289 passed and13 explicit opt-in skips, no failures; Ruff passed |
| Latest frozen Ubuntu source, including installer group termination, shared-runtime admission and inherited force-prefix filtering |320 Python tests ran:306 passed and14 skipped, no failures; Ruff passed. Skips comprise13 opt-in checks and1 case-insensitive-filesystem-only check |
| Latest Ubuntu installer signal/timeout acceptance |7 focused groups passed within the normal suite: TERM/HUP/INT, ignored/repeated signals, remaining grandchild, spawn races/failure, timeout and non-main-thread rejection |
| Latest Ubuntu shared-runtime admission |11 groups ran:10 passed,1 case-insensitive-parent check skipped on the case-sensitive Linux filesystem; no failures. Concurrent readers exclude repair, and different homes cannot mutate an active shared runtime |
...
An earlier Ubuntu session/ACK attempt initially stopped because the container source allowlist omitted the newly added public `monitoring/sync-status-manifest.v1.json`; its artifact parity test failed before upstream installation. The allowlist now explicitly includes the public `monitoring/` directory. That corrected snapshot completed successfully. The later268-,278-,288- and302-test snapshots completed their explicit stages and removed their containers; Docker confirmed each container's absence. The current320-test snapshot used two attempts as detailed below. Ordinary-suite opt-in skips are reported separately from the explicit runs and are not counted as passed. The separate four-skill discovery/inference opt-in was not run in the latest Ubuntu invocation; its earlier macOS evidence remains distinct from the new header/discovery probe. The identity tests use the OS temporary directory instead of assuming macOS `/private/tmp` exists.
...
The latest frozen320-test snapshot copied112 Public files with ledger SHA-256 `a42b1d361f5ec8b80bc13f8df49f66e681b942956534850ce71899328724c1aa`. New files were `runtime_lock.py`, `test_runtime_commands.py` and `test_runtime_lock.py`. The normal suite passed306 tests with13 opt-in skips and1 filesystem-conditional skip; actual native Secret Service checks passed2 separately. The inherited force-prefix regression's seven additional fixtures passed inside the existing runtime environment test. These are overlapping subsets, not extra unique-test counts.

The first official-install attempt then returned the fixed `runtime_command_failed` error (script exit3). It did not retain the failed subprocess phase, so its cause remains unidentified; it is not described as a proven network transient. A second fresh container copied the byte-identical112-file snapshot and repeated only the unfinished official stage. A fixture observer recorded fixed phase/exit metadata without changing subprocess arguments or results. Fresh CLI `setup` followed by the production CLI `install-runtime` succeeded through the shared-target lock and new installer lifecycle: Python check, Git clone/checkout, exact-pin verification, venv creation and pip all returned0. The fixture's UID was10001 and HOME was its container home, not `/root`. No enrollment, native installation identity or service API key was supplied.

All nine official selections then passed with zero skips: runtime relay11, environment4, auxiliary2, terminal credential scrub1, runtime PTY1, managed signals3, strict header/discovery5, oneshot1 and memory-exit1. The source ledger inside the second container was identical before/after these checks and matched the frozen host source; the verified Git HEAD remained the exact pin. Both containers had `privileged=false`, no mounts or published ports, a3GiB memory limit,2 CPUs and256 process limit, and both exact IDs were confirmed absent after cleanup. This latest run copied no Private files and used no external inference.

Evidence is under `/private/tmp/myhermes-ubuntu-freeze-20260911/` (combined `report.json`, initial log, source ledger/delta and cleanup check) and `/private/tmp/myhermes-ubuntu-freeze-retry-20260911/` (successful official log, fixed phase diagnostics, matching before/after ledgers and cleanup check). The script now exercises fresh CLI setup/install rather than calling the low-level installer directly. Only that script and this OS-results document changed during this validation; the tested35 Python modules and packaged assets remained frozen. The earlier generic process-death driver and cross-OS51-frame protocol were not repeated.

The installed Ubuntu Git `HEAD` is exactly `2237be355906fbe6065ce1815711eee52b2d646e`; an earlier CLI probe reported Hermes0.21.1/release2026.9.7, Python3.12.3 and OpenAI SDK2.24.0. Use Git `HEAD` for the pin check; the CLI's update-related `upstream` display is not that immutable pin. Test totals describe the executed source snapshot; later added tests require a new run before updating totals.
