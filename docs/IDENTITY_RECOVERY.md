...
1. `re-enroll --dry-run` reads only bounded metadata: no files, native keys or network requests are created. Real operations require the owner's controlling terminal and explicit confirmation.
   A disposable synthetic PTY must exercise the actual `/dev/tty` prompt for replacing an identity, cancelling an unpublished candidate and declining either action; replacing the prompt with a test double does not verify native terminal compatibility.
   Confirmation must validate the complete line `yes`, reject long/padded prefixes, and restore terminal attributes and signal handlers after SIGINT, SIGTERM or SIGHUP. It must fail before changing terminal state on a non-main thread. SIGKILL and an unavailable terminal device cannot guarantee restoration.
...
Native confirmation reads a complete bounded line directly from the terminal descriptor. Type exactly `yes` and Enter; padded or long prefixes are not accepted. The prompt temporarily enables canonical input, discards previously queued input, and restores the original terminal mode afterwards. SIGINT, SIGTERM and SIGHUP interrupt through the same restoration path, including restoring previous signal handlers. Calls from another thread fail before terminal state changes. SIGKILL and loss of the terminal device remain outside this guarantee.

## Local evidence

Four real controlling-terminal tests now cover replacement/candidate cancellation/decline, long and padded confirmation rejection, SIGINT/SIGTERM/SIGHUP restoration, and non-main-thread refusal. The unsafe buffered update mode initially failed to open a real `/dev/tty`; a later independent probe showed truncated `yes` prefixes being accepted. Both defects were reproduced before the corrections. The complete identity selection on macOS/Python 3.13 reported `Ran 22 tests in 0.950s`, `OK`, with no skips. All terminal input and credentials in these tests are synthetic and captured internally; no real owner approval or native key is involved.
