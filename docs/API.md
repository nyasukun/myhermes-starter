...
Browser identity-binding acceptance: a page bootstraps with `GET /v1/me` without an identity header, pins that response's `person_id`, then sends `X-MyHermes-Person-Id` on every subsequent `/v1/` or `/llm/` API request, including reads and later `/v1/me` requests. The value is an exact, case-sensitive opaque identifier, 1–128 characters matching `[A-Za-z0-9][A-Za-z0-9._:-]{0,127}`. Missing or empty values return `428 browser_identity_required`; malformed values or a different authenticated person return `409 browser_identity_changed`. Only an absent header on the initial `GET /v1/me` is a bootstrap exception. Compare immediately after authentication, before reading an application request body, invoking content storage or performing application mutations. A same-person retry retains its original update ID and payload; a browser must stop rather than replay a previous person's draft under a new identity. This header is a precondition, never an owner selector or authorization credential: verified authentication still determines the namespace and current permissions.

Static assets, unauthenticated enrollment creation, proof-authenticated enrollment completion/token issuance and registered-device DPoP requests do not require this browser header. The browser enrollment description and approval endpoints do require it. The server does not turn a failed precondition into a new-person retry or echo the supplied/actual identifier in an error. Browser clients must deploy with this admission requirement; the device API and JSON payload schemas are unchanged.

An HTTP stack may normalize an empty header to absence. On bootstrap `GET /v1/me`, that is indistinguishable from an absent header; other API requests still require a matching nonempty value.
...
1. `POST /v1/enrollments`: `{schema_version:"1",label,os:"ubuntu"|"macos",arch,client_version,public_jwk}`. Response includes `enrollment_id`, `user_code`, `verification_uri`, `expires_at`. Do not send private key material. The request expires after a short registration window.
   ...
   Enrollment-response acceptance (recorded before the additional validation): the response has exactly those four fields, its one-time code is 16 uppercase hexadecimal characters, and its approval URI contains only the matching enrollment ID on the configured origin. Validate the whole response before persisting its code or metadata, opening a browser or writing to the owner's terminal. Validate a resumed native-store code by the same rule. Malformed responses and terminal control sequences must produce a fixed error without storing or displaying their contents. Normal pending/resumed/completed registration remains supported.
