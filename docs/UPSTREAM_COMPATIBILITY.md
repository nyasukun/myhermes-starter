...
This initial compatibility investigation inspected the official release metadata and pinned source. It did not install or execute the full Hermes runtime, call an LLM provider, or authorize a real service account. The companion itself accepts Python 3.14, but the pinned Hermes runtime does not. The latest inspected official release, `v2026.9.14`, still declares `requires-python = ">=3.11,<3.14"` because some Rust-backed dependencies lack compatible 3.14 wheels ([official pinned release metadata](https://github.com/NousResearch/hermes-agent/blob/v2026.9.14/pyproject.toml)). Therefore run the companion in the normal `python3` environment, and supply a separate Python 3.11–3.13 interpreter to `install-runtime --python` and `upgrade --python`. Neither Ubuntu nor macOS end-to-end Hermes operation under Python 3.14 is established by companion-only compatibility.
...
## Managed Desktop adapter (2026-09-24)

The macOS native integration keeps the original pin clean and builds a separate, hash-checked source copy. The exact adapted files and policy are public in `desktop-sources.json`, `desktop_adapter.py` and `desktop_policy.py`. Desktop inherits the enrolled home and managed relay for its entire process lifetime. Provider inventory, main/auxiliary resolution, home/profile selection and Electron backend/update routing are adapted at this pin. See [DESKTOP.md](DESKTOP.md); native GUI/production validation is separate from source and synthetic process checks.
