...
export const RELAY_MANIFEST={
  version:'1.3.0',source:'server_observed',body_storage:false,
  ...
  retention:{known_ms:90*86400000,known_clock:'last settlement or outcome update; additionally after embedded ID admission expires',unknown:'indefinite; reservation and frozen state are never cleared by retention',max_records:100000,batch_size:100,admission:{id_version:7,max_age_ms:30*86400000,future_tolerance_ms:300000,legacy:'retained receipts only; absent non-v7 IDs rejected'},idempotency:'Existing receipts checked first; purged IDs cannot be admitted again; persistent clock floor prevents rollback replay; capacity rejects new inference.',coverage:'summary history_complete_since identifies retention-limited history'},
